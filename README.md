...
- to compare **decision cost**, replay the same recorded snapshot through both paths:
  `make replay-plugin` (the `kaptain-replay` binary) and `make replay-extender`
  (`POST /replay`). Both print raw scores, normalised scores and a duration, and the shared
  fixtures check that they agree. This is also the only way to compare inference cost and
  decision stability without the cluster moving underneath.
- in cluster, report the freshness difference rather than assume it away: each extender
  ...
  current. The extender additionally holds its own in-flight decisions until a refresh that
  started after them completes, which is its equivalent of the plugin's `Reserve`.
