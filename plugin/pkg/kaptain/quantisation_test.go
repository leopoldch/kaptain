...
// TestQuantisationMatchesTheExtender walks the shared table in testdata/quantisation.json.
// The Python extender checks the same cases, so a change to either quantisation breaks one
// of the two suites instead of silently making the paths place pods differently.
func TestQuantisationMatchesTheExtender(t *testing.T) {
