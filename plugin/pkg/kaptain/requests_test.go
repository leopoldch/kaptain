...
// TestPodRequestsMatchesTheSharedTable walks testdata/pod-requests.json, which the Python
// extender checks with its own port of the same algorithm. A divergence between the two
// request calculations breaks one of the two suites instead of quietly giving the paths
// different views of the same pod.
