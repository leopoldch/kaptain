...
// Registered on the scheduler's own /metrics. Labels stay low cardinality: no pod UID, no
// node name, no image -- those go to the decision log.
var (
	...
	strategyMismatchTotal = metrics.NewCounterVec(&metrics.CounterOpts{
		Subsystem:      subsystem,
		Name:           "strategy_mismatch_total",
		Help:           "Decisions where the decider named a policy other than the declared one.",
		StabilityLevel: metrics.ALPHA,
	}, []string{"declared", "reported"})
