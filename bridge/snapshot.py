...
# Kubernetes quantity suffixes. Decimal and binary multipliers, plus the sub-unit
# suffixes the metrics API uses: it reports CPU in nanocores ("123456789n").
_SUFFIXES = {
    "n": Fraction(1, 10**9),
    "u": Fraction(1, 10**6),
    "m": Fraction(1, 10**3),
...
def parse_quantity(value: str) -> Fraction:
    """A Kubernetes resource quantity as an exact rational.

    Exact on purpose: the metrics API reports CPU in nanocores, and float arithmetic there
    would drift away from what the scheduler computes. testdata/quantities.json holds the
    reference conversions, generated by apimachinery itself.
    """
    ...
    for suffix in ("Ki", "Mi", "Gi", "Ti", "Pi", "Ei"):
        if text.endswith(suffix):
            return Fraction(text[: -len(suffix)]) * _SUFFIXES[suffix]

    last = text[-1]
    if last in _SUFFIXES:
        return Fraction(text[:-1]) * _SUFFIXES[last]

    # Plain number, or the exponent form Kubernetes also accepts ("1e3", "1500e-3").
    return Fraction(text)


def _ceil(value: Fraction) -> int:
    # Kubernetes rounds a quantity towards +infinity, so 123456789n is 124 millicores.
    return -((-value.numerator) // value.denominator)


def parse_cpu(value: str) -> int:
    """Kubernetes CPU quantity to millicores, rounded as Kubernetes rounds."""
    return _ceil(parse_quantity(value) * MILLICORES_PER_CORE)


def parse_memory(value: str) -> int:
    """Kubernetes memory quantity to bytes, rounded as Kubernetes rounds."""
